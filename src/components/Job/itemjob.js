...
export default function ItemJobs({post,index}) {
  // console.log('2nd',post)
  // console.log('er',toString(index))
  // console.log('er',key)
  // det = post.i
  return (
    <div className='flex w-full font-thin bg-white text-gray-500 shadow-sm rounded-sm mb-2'>
      ...
      <div className='flex-1 p-3 flex flex-col'>
        <div className='text-xs self-end'>
          {post.jobPostedAt??post.jobPostedAt}
        </div>
        <h3 className='text-lg text-red-500 py-2'>{post.title??post.title}</h3>
        <div className='flex'>
          ...
          <div className='h-full'>
            <button className='py-1 px-4 border border-red-500 rounded-sm text-red-500 hover:text-white hover:bg-red-500 transition ease-in-out'>
              <Link  to={"details" +"/" + "job"} state={{post}}  >
                Lire la suite 
              </Link>
